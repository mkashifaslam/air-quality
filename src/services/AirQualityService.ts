...
export class AirQualityService {
  ...
  async getNearestAirQuality(
    lat: number = 48.856613,
    lng: number = 2.352222
  ): Promise<{ Result: Result }> {
    try {
      const params = `&lat=${lat}&lon=${lng}`;
      const apiResponse = await this.httpService.axiosRef.get(`${this._apiUrl}${params}`)
  ...
  async createAirQuality(data: Prisma.AirQualityCreateInput): Promise<AirQuality> {
    try {
